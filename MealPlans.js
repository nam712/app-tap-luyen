    ...
    <ScrollView style={styles.container}>
      ...
      <View>
        {mealplanData.map((meal) => (
          <View key={meal.name} style={styles.itemWrapper}>
            <TouchableOpacity onPress={() => setSelectedMeal(meal.name)}>
              <Icon name={selectedMeal === meal.name ? 'checkmark-circle' : 'ellipse-outline'} size={24} color={selectedMeal === meal.name ? '#1E90FF' : '#888'} style={styles.selectionIcon} />
            </TouchableOpacity>
            <TouchableOpacity style={styles.itemCard} onPress={() => setSelectedMeal(meal.name)} >
              <View style={styles.itemTextContainer}>
                <Text style={styles.itemName}>{meal.name}</Text>
                <View style={styles.inlineInfo}>
                  <Icon name="time-outline" size={16} color="#555" />
                  <Text style={[styles.itemInfoText, { marginLeft: 3 }]}>{meal.duration}</Text>
                  <Icon name="flame-outline" size={16} color="#555" style={{ marginLeft: 10 }} />
                  <Text style={[styles.itemInfoText, { marginLeft: 3 }]}> {meal.calories} </Text>
                </View>
              </View>
              <Image source={meal.image} style={styles.itemImage} />
              <TouchableOpacity style={styles.favoriteItem} onPress={() => toggleFavorite(meal)}>
                <Icon name={favorites.some(fav => fav.name === meal.name) ? "heart" : "heart-outline"} size={24} color="red" />
              </TouchableOpacity>
            </TouchableOpacity>
          </View>       
        ))}
      </View>
      <TouchableOpacity style={styles.recipeButton} onPress={() => { const selected = mealplanData.find((meal) => meal.name === selectedMeal); 
        if (selected) { navigation.navigate('RecipeDetail', { meal: selected }); } }} disabled={!selectedMeal}>
